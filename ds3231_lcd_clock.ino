...
BH1750FVI::eDeviceMode_t DEVICEMODE = BH1750FVI::k_DevModeContHighRes2;     //BH1750光照
...
void refreshDisplay() {
  ...
  float innerTemp = rtc.readTempRegister();
  float outerTemp = sensors.getTempCByIndex(0) - 2.55; //貌似18b20有误差..?
  //最大程度避免刷新闪屏
  lcd.setCursor(0, 0);
  lcd.print("                    "); //20个空格
  lcd.setCursor(0, 0);
  ...
  lcd.print("                    ");
  lcd.setCursor(0, 2);
  lcd.print("In:" + String(innerTemp) + "C");
  lcd.setCursor(0, 3);
  lcd.print("                    ");
  lcd.setCursor(0, 3);
  switch (message)
  {
    case 1: //正常显示外部温度
      {
        lcd.print("out:" + String(outerTemp) + "C");
        break;
      }
    case 2: //连接wifi中
      {
...
void setup() {
  ...
  if (EEPROM.read(0) != 1) {      //第一次启动
    ...
    delay(500);
    WiFi.mode(WIFI_STA);
    //smartconfig_start(updateConfigStatus);
